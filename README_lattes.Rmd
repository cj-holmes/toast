...
* After seeing [this viral repo](https://github.com/cj-holmes/toast) about the best way 'to sort a toast', I thought I'd have a go replicate it to sort lattes.
...
* Manually pick x and y values that will get me somewhere near the center of each latte
* I'm using the fact that the lattes are roughly on a uniform grid to allow me to choose only 12 x-values and 10 y-values
...
* Generate the labels for each latte and define the width `w` and height `h` of the selection box to apply to each latte
